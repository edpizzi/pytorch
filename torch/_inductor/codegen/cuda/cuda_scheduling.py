...
class CUDASchedulerNode(SchedulerNode):
    ...
    def fuse_epilogue(self, other_node: SchedulerNode) -> FusedSchedulerNode:
        ...
        cnode: CUDATemplateBuffer = cast(CUDATemplateBuffer, self.node)
        fused_template_buffer = cnode.create_fused_buffer(other_node.node)
        template_node = CUDASchedulerNode(
            self.scheduler, fused_template_buffer, self.group_fn
        )
        fused_node = FusedCUDASchedulerNode(template_node, self.scheduler, [other_node])
        return fused_node
...
class FusedCUDASchedulerNode(FusedSchedulerNode):
    def __init__(
        ...
        assert isinstance(template_node, CUDASchedulerNode)
        assert (
            template_node not in epilogue_scheduler_nodes
        ), "template node should not be in snodes"
        assert template_node.is_template()
        super().__init__(scheduler, [template_node] + epilogue_scheduler_nodes)
    ...
    def fuse_epilogue(self, other_node: SchedulerNode) -> FusedSchedulerNode:
        assert self.can_fuse_epilogue(other_node)
        # Non-mutating, the fused nodes are new instances
        tnode = cast(CUDATemplateBuffer, self.get_cuda_scheduler_node().node)
        fused_template_buffer = tnode.create_fused_buffer(other_node.node)
        template_node = CUDASchedulerNode(
            self.scheduler,
...
class CUDAScheduling(TritonScheduling):
    ...
    def codegen_template(self, template_node, epilogue_nodes):
        ...
        assert rnumel == 1
        ctb = cast(CUDATemplateBuffer, template_node.node)
        assert {id(n) for n in epilogue_nodes} == {
            id(n) for n in ctb.epilogue_nodes
